...
- The final warm four-phase smoke on the dual-index source measured 0.833% disabled baseline, 0.894% enabled idle, 1.100% normal pre-flow and 0.833% stress/churn service CPU. Normal is below 3%, UI remained online/responsive, RAM was bounded and every phase ended with zero test session/marker. The earlier 3.938% baseline run is explicitly noisy/invalid and is not reused.
