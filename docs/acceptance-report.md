...
Phases 1–3 now have verified DPI 100%, DPI 150%, direct tray interaction and the earlier scoped reboot/soak evidence, but the current PR head has not been built, published and reboot-tested as the final artifact. **Do not begin Phase 4/ETW yet.** Restore the original 125% display scale manually, then perform one final-artifact reboot acceptance. Production signing/approval must also be resolved before any release claim.
