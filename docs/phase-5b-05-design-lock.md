...
### Snapshot framing re-lock and corrected maximum evidence
...
and accepting prompt expiry retain their required `true` values. Authorization
uses all five valid `false` values; every optional field is present. Statuses use
the compatible `Blocked`/`TrafficFailedOpen == false` combination. Critical
...
1,261,143 bytes and already exceed the frame. The canonical JSON-safe ASCII
alphabet above is therefore part of the framing bound, not an
optional UI policy. The trade-off is deliberate: localized presentation text
is produced by the UI from stable reason codes, non-ASCII filenames use synthetic redacted
