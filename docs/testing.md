...
`tools/Validate-EgressGuard.ps1` is the single entry point used locally and by CI. It verifies `EgressGuard.sln` exists, then runs, in order: `dotnet tool restore`, locked solution restore, `dotnet build-server shutdown`, format verification (never rewrites files), Release build, the executable test suite, the vulnerable-package audit and `git diff --check`; it then displays `git status --short`, validates the structure of `AGENT_HANDOFF.md` (size and line limits, required sections, coordination-issue link, a final newline, common secret patterns) and finally enforces `-RequireClean`. It stops at the first failing step and exits with that step's exit code. The script locates the repository root from its own location, needs no administrator rights and no GitHub access token, never reads or updates GitHub issues, changes no firewall/Defender/registry/service state, never modifies source or other Git-tracked files, avoids `Invoke-Expression`, and never prints matched secret values. Note that `dotnet tool restore`, `dotnet restore` and the vulnerable-package audit may contact NuGet over the network. Tool restore, build and tests create only Git-ignored `bin/`, `obj/` and package-cache artifacts.

`-RequireClean` requires the working tree to be clean at the end of the run, including tracked changes that already existed before starting; untracked and Git-ignored output does not count. Run without it during active development; CI and any clean-checkout verification always pass `-RequireClean`.
...
Agent sessions coordinate through [AGENT_HANDOFF.md](../AGENT_HANDOFF.md) and the coordination issue linked inside it; the issue holds the live session log while this static guide stays unchanged between branches.
