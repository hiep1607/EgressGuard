...
PowerShell firewall operations use a finite internal timeout. Once an owned child starts, stdout, stderr and exit are observed together; cancellation, timeout or I/O failure terminates only that process tree and waits for cleanup with an independent timeout. An indeterminate create is reconciled by exact rule name, full expected ownership description, program, direction and action. A matching rule newly created by the failed request is removed; a mismatched rule is never deleted. Caller cancellation is rethrown after cleanup.
