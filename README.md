...
For a local development run, use `powershell -ExecutionPolicy Bypass -File .\tools\Start-EgressGuardLocal.ps1`. It starts a source-built Service and UI with a per-run data directory and Named Pipe, waits for the private pipe, and cleans only the process trees it created when the UI closes. It does not install a Windows Service or change firewall, registry, Defender, or other security policy. File activity tracking can require an appropriately privileged Service token; if permission is unavailable, EgressGuard reports that state and continues network monitoring. Enable or disable it in Settings, then restart the Service for the saved choice to take effect.
