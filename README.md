...
The **File Activity History** tab reads only file activity already correlated with an observed network connection; it is not a complete file inventory. The Service endpoint is cursor-paginated and bounded, while the UI keeps the end timestamp fixed during refresh/load-more/export. CSV export runs in the UI, uses UTF-8 with BOM, escapes CSV fields, neutralizes spreadsheet formulas, and caps reports at 5,000 rows. Reports contain redacted display identifiers only and never file contents, real paths, connection addresses, or protected file identifiers.

For a local development run, use `powershell -ExecutionPolicy Bypass -File .\tools\Start-EgressGuardLocal.ps1`. It builds the solution synchronously, starts the built Service and UI DLLs with a per-run data directory and Named Pipe, waits for the private pipe, and cleans up only the direct Service and UI processes it created when the UI closes. It does not install a Windows Service or change firewall, registry, Defender, or other security policy. File activity tracking can require an appropriately privileged Service token; if permission is unavailable, EgressGuard reports that state and continues network monitoring. Enable or disable it in Settings, then restart the Service for the saved choice to take effect.
